@extends('layouts.app', ['activePage' => 'login', 'title' => __('Home')])
      ...
      <form class="form" method="POST" action="{{ route('login') }}">
        ...
        <div class="card card-login card-hidden mb-3">
          ...
          <div class="card-body">
            ...
            <div class="bmd-form-group{{ $errors->has('email') ? ' has-danger' : '' }}" style="padding-top: 2rem">
        ...
        <div class="col-6">
            @if (Route::has('password.request'))
                <a href="{{ route('password.request') }}" class="text-light">
                    <small class="text-primary">{{ __('Forgot password?') }}</small>
                </a>
            @endif
        </div>
        <div class="col-6 text-right">
            <a href="{{ route('register') }}" class="text-light">
                <small class="text-primary">{{ __('Create new account') }}</small>
            </a>
        </div>
      </div>
    </div>
